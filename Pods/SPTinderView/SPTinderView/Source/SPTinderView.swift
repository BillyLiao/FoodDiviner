...
public class SPTinderView: UIView, UIGestureRecognizerDelegate{
    ...
    public override init(frame: CGRect) {
        ...
        reloadData()
        tapGesture = UITapGestureRecognizer(target: self, action: "cellDidTapped")
    ...
    func cellDidTapped(){
        if currentIndex >= numberOfCells { return }
        else {
            if let _delegate = self.delegate {
                _delegate.tinderView(self, didTappedCellAt: currentIndex)
            }
        }
    }
    ...
    */
    ...
    public func animateRemovalForCell(cell: SPTinderViewCell, towards direction: SPTinderViewCellMovement, completion:()->()) {
        ...
        case .Bottom: newPosition = CGPoint(x: cell.center.x, y: cell.center.y)
        }
        
        // If direction == Bottom, then don't remove the cell
        UIView.animateWithDuration(0.3, animations: {
